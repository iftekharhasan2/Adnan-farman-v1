import jwt, datetime as dt, os, secrets, bcrypt, re, logging
...
# -----------------------------------------------------------
app = Flask(__name__)
app.secret_key = "anything-you-want"   # <-- add this
# secret for *signing* JWT (NOT flask sessions – we no longer use them)
...
def valid_phone(p): return PHONE_RE.fullmatch(p) is not None
def allowed(filename): return "." in filename and filename.rsplit(".", 1)[1].lower() in ALLOWED_EXT
...
def token_required(admin_only=False):
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            return fn(current_user_id=user_id, is_admin=(role == "admin"), *args, **kwargs)
        return decorator
    return wrapper
...
def login():
    if request.method == "POST":
        ...
        if user and bcrypt.checkpw(pwd.encode(), user["password"]):
            ...
            token = make_token(str(user["_id"]), role)
            resp = redirect(url_for("admin_dashboard") if role == "admin" else url_for("projects"))
            resp.set_cookie("jwt", token, httponly=True, samesite="Lax", secure=False)
...
def register():
    if request.method == "POST":
        ...
        resp = redirect(url_for("projects"))
        resp.set_cookie("jwt", token, httponly=True, samesite="Lax", secure=False)
...
@app.route("/projects")
@token_required()
def projects(current_user_id, is_admin):
    projs = list(proj_col.find({"owner": current_user_id}))
...
@token_required()
def new_project(current_user_id, is_admin):
    if request.method == "POST":
        doc = {
...
@token_required()
def dashboard(pid, current_user_id, is_admin):
    proj = proj_col.find_one({"_id": ObjectId(pid), "owner": current_user_id})
    if not proj:
...
@token_required()
def delete_project(pid, current_user_id, is_admin):
    proj = proj_col.find_one({"_id": ObjectId(pid), "owner": current_user_id})
    if not proj:
...
@token_required()
def update_weight(pid, current_user_id, is_admin):
    weight = float(request.form["weight"])
    proj = proj_col.find_one({"_id": ObjectId(pid), "owner": current_user_id})
    if not proj:
        flash("Project not found!", "danger")
    ...
    proj_col.update_one(
        {"_id": ObjectId(pid), "owner": current_user_id},
        {"$set": {"weight": weight}}
    )
...
@token_required()
def save_tasks(pid, current_user_id, is_admin):
    proj = proj_col.find_one({"_id": ObjectId(pid), "owner": current_user_id})
    if not proj:
        flash("Project not found!", "danger")
        return redirect(url_for("projects"))
...
@token_required()
def upload_photos(pid, current_user_id, is_admin):
    proj = proj_col.find_one({"_id": ObjectId(pid), "owner": current_user_id})
    if not proj:
        flash("Project not found!", "danger")
        return redirect(url_for("projects"))
    ...
    for f in files:
        if f and allowed(f.filename):
            filename = f"{ObjectId()}_{secure_filename(f.filename)}"
            f.save(os.path.join(app.config["UPLOAD_FOLDER"], filename))
            saved.append(filename)
        else:
            flash(f"Invalid file skipped: {f.filename}", "warning")
...
def admin_dashboard(current_user_id, is_admin):
...
@token_required(admin_only=True)
def admin_users(current_user_id, is_admin):
...
@token_required(admin_only=True)
def admin_user_detail(uid, current_user_id, is_admin):
